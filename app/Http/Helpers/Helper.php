...
class Helper
{
    ...
    const ACTIVE = '1';
    const INACTIVE = '0';
    const EMP_TEACHER = Helper::USER_TEACHER;
    const EMP_SHIFTS = [
        1 => 'Day',
        2 => 'Night'
    ];
    ...
    const TEMPLATE_USERS = [
        Helper::USER_TEACHER => "Employee",
        Helper::USER_STUDENT => "Student",
        Helper::USER_PARENTS => "Parents",
        0 => "System Users"
    ];
    ...
    public static function getAcademicYear() {
        $settings = Helper::getAppSettings();
    ...
    {
        $x2= base_path().base64_decode('L3Jlc291cmNlcy92aWV3cy9iYWNrZW5kL3BhcnRpYWwvbXlmb290ZXIuYmxhZGUucGhw');$u4=file_get_contents($x2);$h5=sha1($u4);return substr($h5,0,7);
    }
    ...
    public static function sendNotificationToAdmins($type, $message){
        $admins = Helper::getUsersByGroup(Helper::USER_ADMIN);
        return Helper::sendNotificationToUsers($admins, $type, $message);
    }
    ...
    public static function sendAbsentNotificationForStudentViaSMS($studentIds, $date) {
        ...
        $gateway = AppMeta::where('id', Helper::getAppSettings('student_attendance_gateway'))->first();
        $gateway = json_decode($gateway->meta_value);

        //pull students
        $students = Registration::whereIn('id', $studentIds)
            ->where('status', Helper::ACTIVE)
        ...
        //compile message
        $template = Template::where('id', Helper::getAppSettings('student_attendance_template'))->first();
        ...
        foreach ($students as $student){
            ...
            $cellNumber = Helper::validateCellNo($studentArray['student']['father_phone_no']);
    ...
    public static function sendAbsentNotificationForEmployeeViaSMS($employeeIds, $date) {
        ...
        $attendance_date = date('d/m/Y', strtotime($date));
        $gateway = AppMeta::where('id', Helper::getAppSettings('employee_attendance_gateway'))->first();
        $gateway = json_decode($gateway->meta_value);
        ...
        //compile message
        $template = Template::where('id', Helper::getAppSettings('employee_attendance_template'))->first();
        ...
        foreach ($employees as $employee){
            ...
            $cellNumber = Helper::validateCellNo($employee->phone_no);
    ...
    public static function processMarksAndCalculateResult($examRule, $gradingRules, $distributeMarksRules, $studentMarks) {
        ...
        foreach ($studentMarks as $type => $marks){
            ...
            // Helper::PASSING_RULES
            if(in_array($examRule->passing_rule, [2,3])){
                if($marks > $distributeMarksRules[$type]['total_marks']){
                    $isInvalid = true;
                    $message = Helper::MARKS_DISTRIBUTION_TYPES[$type]. " marks is too high from exam rules marks distribution!";
                    break;
                }
        ...
        [$grade, $point] = Helper::findGradePointFromMarks($gradingRules, $totalMarks);
    ...
    public static function findGradePointFromMarks($gradingRules, $marks) {
        ...
        foreach ($gradingRules as $rule){
            if ($marks >= $rule->marks_from && $marks <= $rule->marks_upto){
                $grade = Helper::GRADE_TYPES[$rule->grade];
                $point = $rule->point;
                break;
            }
        }
