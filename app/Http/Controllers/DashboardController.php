...
//        return $collectionsAll;
        $properties = project::all();

        return view('dashboard',compact('collections','totalDue','total','newRenters','collectionsAll', "properties"));
    }
...
//        if(!\Session::has('dueNotifications') || !count(session('dueNotifications'))){
        ...
        if(!$haveDueNotification){
            $collectionsHave = RentCollection::select('rents_id')->whereYear('collectionDate', '=', date('Y'))
                ->pluck('rents_id');
