...
//      return $pdf->stream($fileName);
    ...
    public function landlord_properties(Request $request)
    {
        ...
        $projects = $query->with('landlord')->orderBy('created_at', 'desc')->get();

//       $pdf = PDF::loadView('report.projects',compact('projects'));
//      $fileName='projects.pdf';
//      return $pdf->stream($fileName);
        return view('report.landlord_properties',compact('projects','projectType','landlord','name','landlords'));
    }
    ...
    public function landlords(Request $request)
    {
        ...
        $query = $query->orderBy('created_at','desc')->with('entry');
        $landlords = $query->get();
        return view('report.landlords',compact('landlords','status','name','mobileNo'));
    }
    ...
    public function collections(Request $request)
    {
        ...
        $reportTitle = "all properties and tenants";
...
//        $toDate = Carbon::createFromFormat('Y-m-d', $toDate)->format('d/m/Y');
    ...
    public function expenses(Request $request)
    {
        ...
        $reportTitle = "All Expenses on Properties";
    ...
    public function remittances(Request $request)
    {
        ...
        $reportTitle = "all Remittances on Properties";

        if($project != "All") {
            $projectInfo = project::where('id',$project)->first();
            $reportTitle = $projectInfo->name;
            $remittances = Remittance::where('projects_id',$projectInfo->id)
                ->where('entryDate','>=',$fromDate)->where('entryDate','<=',$toDate)
                ->orderBy('created_at','desc')->with('project')->with('entry')->get();
        }
        else{
            $remittances = Remittance::where('entryDate','>=',$fromDate)->where('entryDate','<=',$toDate)
                ->orderBy('created_at','desc')->with('project')->with('entry')->get();
        }
        $fromDate = Carbon::createFromFormat('Y-m-d', $fromDate)->format('d/m/Y');
        $toDate = Carbon::createFromFormat('Y-m-d', $toDate)->format('d/m/Y');
        return view('report.remittances',compact('remittances','project','projects','fromDate','toDate','reportTitle'));
    }
